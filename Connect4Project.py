...
def get_C_score_for_this_position(player, board):
    ...
    center_R_col = board[:,(COLS//2)-1]
    score +=list(center_R_col).count(player) * 1
    center_L_col = board[:,(COLS//2)+1]
    score +=list(center_L_col).count(player) * 1
    return score
...
def give_score_to_line(line, player):
    ...
    if line.count(player) == 4:
        ...
        score -= 8
    elif line.count(opp) == 2 and line.count(FREE) == 2:
        score -= 2
    return score
